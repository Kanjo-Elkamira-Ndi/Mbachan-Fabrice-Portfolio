...
const blank: Omit<ExperienceItem, "id"> = {
  organization: "", role: "", period: "", location: "", highlights: [],
};
      ...
      <ResourceCrud<ExperienceItem>
        ...
        renderForm={(s, set) => (
          <>
            ...
            <Field label="Period"><input className={inputCls} value={s.period} onChange={(e) => set({ ...s, period: e.target.value })} /></Field>
            <Field label="Location"><input className={inputCls} value={s.location} onChange={(e) => set({ ...s, location: e.target.value })} /></Field>
            <Field label="Highlights (one per line)">
              <textarea rows={4} className={inputCls} value={s.highlights.join("\n")} onChange={(e) => set({ ...s, highlights: e.target.value.split("\n").map((x) => x.trim()).filter(Boolean) })} />
            </Field>
          </>
        )}
      />
